...
parser.add_argument('--percent_act_shuffle', type=str, help='percentage of the activations shuffled')
...
q =  int(args.percent_act_shuffle)/100 #0.4# fraction shuffled
max_  = 2.5

num_cell_shuffle = int(5000 * q)  
...
rand_ind = random.sample(list(range(5000)), num_cell_shuffle) 
activation_matrix[0, rand_ind] =   torch.rand((num_cell_shuffle)) * max_ 
rand_ind = random.sample(list(range(5000)), num_cell_shuffle)
activation_matrix[1, rand_ind] =   torch.rand((num_cell_shuffle)) * max_ 
rand_ind = random.sample(list(range(5000)), num_cell_shuffle)
activation_matrix[2, rand_ind] =   torch.rand((num_cell_shuffle)) *  max_ 
rand_ind = random.sample(list(range(5000)), num_cell_shuffle)
activation_matrix[3, rand_ind] =   torch.rand((num_cell_shuffle)) * max_ 
...
data += torch.randn(2000, 5000)*(0.10)- 0.05
data = data.abs()
...
num_cells = 5000
num_genes = 2000
adata.write('data/sim_anndata_linear_model_' + str(num_cells)+'_cells_'+str(num_genes)+'_genes_'+str(num_facs)+'_factors_'+str(seed)+'_seed_'+args.percent_act_shuffle+'_percent_act_shuffle.h5ad')
