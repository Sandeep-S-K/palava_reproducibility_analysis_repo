...
lam = [0.5] * num_facs_ann + [0.5/2] * num_facs_unann
...
scvi_palava = SCVI_palava(adata, n_annotated_latent =  len(pathways_bool), n_unannotated_latent = num_facs_unann, nonlinear = nonlinear, palava_n_hidden = 1000, other_n_latent = 20, lam_lst = lam, pathways_bool = pathways_bool, dispersion = 'gene',   n_layers = 3, use_batch_norm = 'none',use_layer_norm = 'none', momentum_train = 0.1,  weight_decay_train = 0.0001, decoder_bias= True,  non_neg_decoder = True)

start = timeit.default_timer()

scvi_palava.train(1000, plan_kwargs = plan_kwargs, batch_size = 100, gradient_clip_val = 1e3)
...
results['factor_loadings_0.5q'] =  np.vstack([np.quantile(abs(slopes_for_all_fac[fac]), q = 0.5, axis = 0) for fac in range(num_facs_unann + num_facs_ann)])
results['factor_loadings_0.65q'] =  np.vstack([np.quantile(abs(slopes_for_all_fac[fac]), q = 0.65, axis = 0) for fac in range(num_facs_unann + num_facs_ann)])
results['factor_loadings_0.75q'] =  np.vstack([np.quantile(abs(slopes_for_all_fac[fac]), q = 0.75, axis = 0) for fac in range(num_facs_unann + num_facs_ann)])

results['factor_loadings_0.85q'] =  np.vstack([np.quantile(abs(slopes_for_all_fac[fac]), q = 0.85, axis = 0) for fac in range(num_facs_unann + num_facs_ann)])
