...
parser.add_argument('--lambda_', type=str, help='sparsity param')
...
lam = [lambda_] * num_facs_ann + [lambda_] * num_facs_unann

nonlinear  = [0 for _ in range(num_facs_ann + num_facs_unann)]


scvi_palava = SCVI_palava(adata, n_annotated_latent =  len(pathways_bool), n_unannotated_latent =num_facs_unann, nonlinear = nonlinear, other_n_latent = 20, lam_lst = lam, pathways_bool = pathways_bool, dispersion = 'gene', palava_n_hidden = 0, palava_n_layers = 0,  n_layers = 3, use_batch_norm = 'none',use_layer_norm = 'none', momentum_train = 0.8,  weight_decay_train = 0.0001, decoder_bias= False,  non_neg_decoder =True)
...
np.save('intermediate_results/linear_palava_'+'lambda_'+args.lambda_+'_results_'+name_of_anndata.replace('.h5ad','.npy'), results, allow_pickle=True)
