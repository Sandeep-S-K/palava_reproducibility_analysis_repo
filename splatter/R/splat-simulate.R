...
splatSimPathCellMeans <- function(sim, params) {
    ...
    for (idx in seq_along(path.from)) {
        ...
        is.nonlinear <- rowData(sim)[[paste0("LocalDEFacPath", idx)]]!=1 #as.logical(rbinom(nGenes, 1, path.nonlinearProb))
...
bridge <- function(x = 0, y = 0, N = 50, n = 100, sigma.fac = 0.8) {
    dt <- 1 / (N - 1)
    t <- seq(0, 1, length = N)
   #sigma2 <- max(c(runif(1, 0, sigma.fac * mean(c(x, y))), 10.0))
    if (sigma.fac * mean(c(x, y))==0) {
      sigma2 <-0
    } else {
      sigma2 <- max(c(runif(1, 0, sigma.fac * mean(c(x, y))), 1.0))
    }

    X <- c(0, cumsum(rnorm(N - 1, sd = sigma2) * sqrt(dt)))
    
    BB <- x + X - t * (X[N] - y + x)
    BB <- stats::spline(BB, n = n, method = "fmm")$y
    BB[BB < 0] <- 1e-6
    return(BB)
}
